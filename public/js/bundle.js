...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoibXVoYW1tYWRhbGltZSIsImEiOiJjbDIwbHk2N2YwMWtiM2lwcHp1OGRhcHBzIn0.6H08UxZ_U7e1mOCmWqzVBw';
  var map = new mapboxgl.Map({
    container: 'map',
    style: 'mapbox://styles/muhammadalime/cl20mj6wi001j14nsbtkcjzj5',
...
var stripe = Stripe('pk_test_51KsgP8KXUKep8lgPxpPzw1mSWghY2bLlpBU1O5yZfYSWvmPaU8a9cVFFUkxXEU2rwz1St1WlPxzldOBzm9pg46vZ0015Gk2gVH');
...
exports.leaveReview = exports.editReview = exports.deleteReview = void 0;
...
var editReview = /*#__PURE__*/function () {
  var _ref2 = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee2(rating, review, reviewId) {
    var res;
    return regeneratorRuntime.wrap(function _callee2$(_context2) {
      while (1) {
        switch (_context2.prev = _context2.next) {
  ...
  return function editReview(_x5, _x6, _x7) {
    return _ref2.apply(this, arguments);
  };
}();
...
var reviewDataForm = document.querySelector('.review--form');
...
if (reviewDataForm) {
  reviewDataForm.addEventListener('submit', function (e) {
    e.preventDefault();
    ...
    (0, _review.leaveReview)(review, rating, tour, user);
    document.getElementById('review').textContent = '';
    document.getElementById('rating').textContent = '';
  });
}

if (reviews) reviews.addEventListener('click', function (e) {
  if (e.target.tagName === 'BUTTON') {
    var button = e.target;
    var reviewsCard = button.closest('.reviews__card');
    var _reviews = reviewsCard.parentNode;

    if (button.textContent === 'Delete') {
      var reviewId = button.dataset.reviewId;
      (0, _review.deleteReview)(reviewId);
      setTimeout(function () {
        _reviews.removeChild(reviewsCard);
      }, 500);
    } else if (button.textContent === 'Edit') {
      var reviewText = reviewsCard.querySelector('.reviews__text');
      var reviewRatingBox = reviewsCard.querySelector('.reviews__rating'); /// Cancel button

      var cancel = document.createElement('button');
      cancel.className = 'review__change review__cancel';
      cancel.id = 'review__cancel';
      cancel.textContent = 'Cancel';
      cancel.setAttribute('data-review-text', reviewText.textContent); /// Find the rating number

      var stars = reviewsCard.querySelectorAll('.reviews__star--active'); // InputReview

      var inputReview = document.createElement('textarea');
      inputReview.style.width = '25.8rem';
      inputReview.className = 'reviews__text';
      inputReview.value = reviewText.textContent; // InputRating
      ...
      var cancelBtn = reviewsCard.querySelector('.review__cancel');
      var editBtn = reviewsCard.querySelector('.review__edit');
      var reviewTextContent = cancelBtn.dataset.reviewText;

      var _inputReview = reviewsCard.querySelector('.reviews__text');

      var _inputRating = reviewsCard.querySelector('.reviews__rating-input');

      var _reviewText = document.createElement('p');

      _reviewText.className = 'reviews__text';
      _reviewText.textContent = reviewTextContent;
      reviewsCard.insertBefore(_reviewText, _inputReview);
      reviewsCard.removeChild(_inputReview);
      reviewsCard.removeChild(_inputRating);
      reviewsCard.removeChild(cancelBtn);
      editBtn.textContent = 'Edit';
    } else if (button.textContent === 'Save') {
      var _inputReview2 = reviewsCard.querySelector('.reviews__text');

      var _inputRating2 = reviewsCard.querySelector('.reviews__rating-input');

      var _cancelBtn = reviewsCard.querySelector('.review__cancel');

      reviewsCard.removeChild(_cancelBtn);

      var _reviewText2 = document.createElement('p');

      _reviewText2.className = 'reviews__text';
      _reviewText2.textContent = _inputReview2.value;
      reviewsCard.insertBefore(_reviewText2, _inputReview2);
      reviewsCard.removeChild(_inputReview2);
      reviewsCard.removeChild(_inputRating2);
      (0, _review.editReview)(+_inputRating2.value, _reviewText2.textContent, button.dataset.reviewId);
